    ...
    implements CommandFunctionTask
{
    ...
    @Getter
    private final String identifier = "TakeItem";
    
    public TakeItem(CommandFunction function, String expression, String configPath) {
    ...
    public void executeTask(CommandSender sender, Map<String, String> placeholders) {
        ...
        if (item != null) {
            item.take(sender, placeholders, target, amount);
        }
    }
    ...
    public static TakeItem build(CommandFunction function, Map map, String configPath) {
        if (map.get("Take-Item") != null) {
            return new TakeItem(function, map.get("Take-Item").toString(), configPath);
        }
        return null;
    }
    
    public static List<TakeItem> build(CommandFunction function, List<String> itemList, String configPath) {
        return itemList.stream().map(item -> new TakeItem(function, item, configPath)).collect(Collectors.toList());
    }
}
